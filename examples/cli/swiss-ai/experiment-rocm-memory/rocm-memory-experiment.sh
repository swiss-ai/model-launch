...
sml advanced \
  --slurm-nodes 1 --serving-framework sglang --slurm-environment "$ENV" \
  --slurm-time "$TIME" --partition mi300 \
...
# 2 nodes, tp-size 8
sml advanced \
  --slurm-nodes 2 --serving-framework sglang --slurm-environment "$ENV" \
  --slurm-time "$TIME" --partition mi300 \
  --framework-args "--model $MODEL \
