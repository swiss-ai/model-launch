...
| ----------------- | ----------------------------------------- | ----------------------------------------------------------------- |
| ≤ 30 B            | 1 GPU                                     | `--replicas N --nodes-per-replica 1`, set framework `--tp-size 1` |
| 30–80 B           | 1 node (4-way TP)                         | 1 replica per node, framework `--tp-size 4`                       |
| 80–250 B          | 1 node (4-way TP) at FP8, or 2 nodes BF16 | quantize, or `--nodes-per-replica 2` + matching TP                |
| 250 B+            | Multiple nodes                            | `--nodes-per-replica 2+`, expect tensor + pipeline parallelism    |
...
| **PP** (pipeline parallelism) | Layers, sharded across GPUs (or nodes) end-to-end                 | Framework flag (e.g. `--pp-size`) inside `--framework-args`. Spans nodes within one replica when `--nodes-per-replica > 1`.      |
...
| **DP** (data parallelism)     | Independent copies serving different requests in parallel         | **`--replicas N`** — N copies of the model, optionally fronted by `--router sglang`.                                             |
...
For dense models (one weight matrix per layer, no MoE routing), DP isn't usually expressed inside the inference framework — you don't tell the framework "give me 4 data-parallel copies on these 4 GPUs". You just request a single GPU per replica and let the **autoscaler** add more replicas when load grows. The orchestrator (k8s, or here, SLURM + `--replicas`) provides DP naturally; the framework only handles TP (and PP when needed).

This shapes the rule below: bump `--replicas` for throughput, not the framework's DP flags.
...
DP across replicas still applies the same way for throughput: more concurrent requests → bump `--replicas`.

## Step 4 — replicas vs. nodes-per-replica

These two flags set very different things:

- **`--replicas N`** — N independent copies of the model. Use for **throughput**: more concurrent requests, optionally fronted by `--router sglang` for load balancing.
- **`--nodes-per-replica K`** — each replica spans K nodes. Use when **one replica doesn't fit on a single node** (large models, long context, more KV cache).

Total nodes = `replicas × nodes-per-replica`.

Rule of thumb:

- Model fits on 1 node, want more throughput? → bump `--replicas`.
- Model doesn't fit on 1 node? → bump `--nodes-per-replica` first, then add replicas if you still need throughput.
...
| Router | **OpenTela** (default, `--router opentela`). The SGLang router (`--router sglang`) adds a hop. |
...
| --- | --- |
| Replicas | **More.** Bump `--replicas` until you hit a partition or budget cap. DP scales linearly. |
| Router | **sglang** (`--router sglang`) for an in-job router across replicas; the default `--router opentela` spreads load across replicas at the OpenTela mesh instead. |
