...
- **Disable OpenTela for raw numbers.** Pass `--disable-opentela` to skip the OpenTela mesh registration on each replica. You then drive load directly to the framework's host:port, which gives you the framework's true throughput. Leave it on for end-to-end numbers that include the mesh + gateway hop. See [When to disable OpenTela](usage-advanced.md#when-to-disable-opentela).
