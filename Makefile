...
format:
	...
	find . -name "*.toml" -not -path "./.venv/*" -not -path "./.claude/worktrees/*" | xargs taplo fmt
	find . \( -name "*.json" -o -name "*.yaml" -o -name "*.yml" \) -not -path "./.venv/*" -not -path "./.claude/worktrees/*" | xargs npx prettier --write
	-npx markdownlint-cli2 --config .markdownlint.yaml --fix "**/*.md" "!.venv/**/*.md" "!.claude/worktrees/**/*.md"
...
shellcheck:
	find . -name "*.sh" -not -path "./.venv/*" -not -path "./.claude/worktrees/*" | xargs uv run --frozen shellcheck

markdownlint:
	npx markdownlint-cli2 --config .markdownlint.yaml "**/*.md" "!.venv/**/*.md" "!.claude/worktrees/**/*.md"

dockerlint:
	find images/ -name "Dockerfile*" | xargs uv run --frozen hadolint

tomlfmt:
	find . -name "*.toml" -not -path "./.venv/*" -not -path "./.claude/worktrees/*" | xargs taplo fmt --check

prettier:
	find . \( -name "*.json" -o -name "*.yaml" -o -name "*.yml" \) -not -path "./.venv/*" -not -path "./.claude/worktrees/*" | xargs npx prettier --check
...
_test-std:
	uv run --frozen pytest -m std --cov --cov-report=term-missing -n 13
